    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        Todo.findById(hexId).then((todo) => {
          expect(todo).toBeFalsy(); //from toNotExist in expect v1 now v21
          done();
        }).catch((e) => done(e));
      });
  });
  ...
  it('should NOT remove a todo', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        Todo.findById(hexId).then((todo) => {
          expect(todo).toBeTruthy(); //remember that toExist is toExist
          done();
        }).catch((e) => done(e));
      });
  });
...
describe('POST /users', () => {
  it('should create a user', (done) => {
    ...
    request(app)
      ...
      .expect(200)
      .expect((res) => {
        expect(res.headers['x-auth']).toBeTruthy(); //toExist is now toBeTruthy
        expect(res.body._id).toBeTruthy();//toExist is now toBeTruthy
      ...
      .end((err) => {
        ...
        User.findOne({email}).then((user) => {
          expect(user).toBeTruthy();//toExist is now toBeTruthy
          expect(user.password).not.toBe(password); //toNotBe doesn't exist anymore with expectv21, there is no equivalent to toNotBe
          done();
        });
      });
  });
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User.findById(users[1]._id).then((user) => {
          expect(user.toObject().tokens[1]).toMatchObject({ //toInclude is no longer in v21, so now we use toMatchObject
            access: 'auth',
