...
//////////      ////  POST  ////  todos   //////
app.post('/todos', authenticate, (req,res) => { //to make the app private we have to add the authenticate middleware
...
app.post('/users/login', (req,res)=>{
  ...
  User.findByCredentials(body.email, body.password).then((user)=> {
    // res.send(user);
    return user.generateAuthToken().then((token)=>{ //return will keep the chain alive
...
app.get('/todos', authenticate, (req,res) => { //just the todos for the currently logged in user
  Todo.find({
    _creator: req.user._id
  }).then((todos) => {
